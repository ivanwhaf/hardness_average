...
parser.add_argument('-project_name', type=str, help='project name',
                    default='cifar10_retrain_hardavg_cut_hardsimple_lrnodecay_9cnn_noise_s0.4')
...
def create_dataloader():
    ...
    # load dataset
    train_set = CIFAR10FromTxt(txt_path="data/cifar10_noise_s0.4_sort.txt", root=args.dataset_path, train=True,
    ...
    return train_loader, test_loader, test_loader, train_set
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader, raw_train_set = create_dataloader()
    ...
    train_set = None
    n = len(raw_train_set)  # 50000

    # main loop
    for epoch in range(args.epochs):
        # simple-hard
        # indices = np.arange((epoch + 1) * int(n / args.epochs))

        # hard-simple
        # indices = np.arange(start=(args.epochs - epoch - 1) * int(n / args.epochs), stop=n)

        # hardness_average
        # indices = np.arange(0)
        # for i in range(args.epochs):
        #     start = i * int(n / args.epochs)
        #     end = i * int(n / args.epochs) + (epoch + 1) * int(n / args.epochs / args.epochs)
        #     tmp = np.arange(start=start, stop=end)
        #     indices = np.append(indices, tmp)

        # hardness_average_cut
        indices = np.arange(0)
        if epoch >= 60:
            for i in range(1, 90):
                start = i * int(n / args.epochs)
        ...
        train_set = Subset(raw_train_set, indices)
    ...
    plt.plot(range(len(train_loss_lst)), train_loss_lst, 'g', label='train loss')
    plt.plot(range(len(train_loss_lst)), val_loss_lst, 'k', label='val loss')
    plt.plot(range(len(train_loss_lst)), train_acc_lst, 'r', label='train acc')
    plt.plot(range(len(train_loss_lst)), val_acc_lst, 'b', label='val acc')
    plt.grid(True)
    plt.xlabel('epoch')
    plt.ylabel('acc-loss')
